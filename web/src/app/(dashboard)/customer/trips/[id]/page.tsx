    ...
    <div style={{ maxWidth:720, margin:"0 auto" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:20 }}>
        ...
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:14, padding:18 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:13, color:"#fbbf24" }}>
            ...
            <a href={`tel:${trip.driverProfile.user.phone}`} style={{
              ...
              fontSize:12, color:"#34d399", textDecoration:"none", fontWeight:600,
            }}>
            ...
            <button onClick={() => setChatOpen(true)} style={{
              ...
              fontSize:12, color:"#a5b4fc", cursor:"pointer", fontWeight:600,
            }}>
              💬 Nhắn tin
            </button>
          </div>
        ...
        <div style={{ background:"var(--bg-surface)", border:`1px solid ${trip.status === "ONGOING" ? "rgba(99,102,241,.35)" : "var(--border-subtle)"}`, borderRadius:16, overflow:"hidden", marginBottom:20, boxShadow: trip.status === "ONGOING" ? "0 0 0 1px rgba(99,102,241,.1)" : "none" }}>
          <div style={{
            ...
            <span style={{ display:"flex", alignItems:"center", gap:8 }}>
              <MapIcon size={15} color="#6366f1"/>
              {trip.status === "ONGOING" ? "Theo dõi tài xế trực tiếp" : "Lộ trình chuyến đi"}
            </span>
            {trip.status === "ONGOING" && (
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                {driverPos ? (
                  <span style={{ fontSize:11, color:"#34d399", fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                  ...
                  <span style={{ fontSize:11, color:"#fbbf24", fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                    <ActivityIcon size={11} color="#fbbf24"/> Chờ GPS tài xế...
                  </span>
                )}
              </div>
            )}
          ...
          {trip.status === "ONGOING" && !driverPos && (
            <div style={{ padding:"10px 16px", fontSize:12, color:"var(--text-muted)", display:"flex", alignItems:"center", gap:6, borderTop:"1px solid var(--border-subtle)", background:"rgba(251,191,36,.04)" }}>
              <ActivityIcon size={12} color="#fbbf24"/>
        ...
        <div style={{
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{
              ...
              background: currentStop.type === "PICKUP" ? "#22d3ee" : "#f472b6",
              boxShadow: `0 0 8px ${currentStop.type === "PICKUP" ? "#22d3ee" : "#f472b6"}`,
            }}/>
            <div>
              ...
              <div style={{ fontSize:12, color:"var(--text-muted)", display:"flex", alignItems:"center", gap:5, marginTop:2 }}>
                <MapPinIcon size={11} color={currentStop.type === "PICKUP" ? "#22d3ee" : "#f472b6"}/>
      ...
      <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:16, overflow:"hidden", marginBottom:20 }}>
        <div style={{ padding:"12px 18px", borderBottom:"1px solid var(--border-subtle)", fontWeight:600, fontSize:14, color:"var(--text-primary)", display:"flex", alignItems:"center", gap:8 }}>
          <MapPinIcon size={15} color="#a78bfa"/> Lộ trình các điểm dừng
        </div>
        <div>
          {stops.map((s, i) => {
            ...
            const pinColor  = s.type === "PICKUP" ? "#22d3ee" : "#f472b6";
            return (
              <div key={s.id} style={{
                ...
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", width:20, flexShrink:0 }}>
                  <div style={{
                    ...
                    background: isDone ? "#34d399" : isSkipped ? "#475569" : isCurrent ? "#6366f1" : "var(--bg-elevated)",
                    border:`2px solid ${isDone ? "#34d399" : isSkipped ? "#475569" : isCurrent ? "#6366f1" : "var(--border-medium)"}`,
                  ...
                  {i < stops.length - 1 && (
                    <div style={{ width:2, flex:1, minHeight:16, background: isDone ? "#34d399" : "var(--border-subtle)", margin:"4px 0" }} />
                  )}
                </div>
                {/* Content */}
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
                    ...
                    {isCurrent && (
                      <span style={{ fontSize:10, background:"rgba(99,102,241,.2)", color:"#a5b4fc",
      ...
      {trip.status === "COMPLETED" && !rated && (
        <div style={{ background:"var(--bg-surface)", border:"1px solid var(--border-subtle)", borderRadius:16, marginBottom:20, overflow:"hidden" }}>
          <div style={{ padding:"12px 18px", borderBottom:"1px solid var(--border-subtle)", fontWeight:600, fontSize:14, color:"var(--text-primary)", display:"flex", alignItems:"center", gap:8 }}>
            <StarIcon size={15} color="#fbbf24"/> Đánh giá tài xế
          </div>
            ...
            <form onSubmit={submitRating}>
              <div style={{ display:"flex", justifyContent:"center", gap:6, marginBottom:18 }}>
                {[1,2,3,4,5].map((n) => (
                  ...
                  >
                    <StarIcon size={32}
                      color={n <= (hoverStar || rating) ? "#fbbf24" : "var(--border-medium)"}
      ...
      {rated && (
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 18px", borderRadius:14, marginBottom:16,
          background:"rgba(52,211,153,.1)", border:"1px solid rgba(52,211,153,.3)", color:"#34d399", fontSize:13, fontWeight:600 }}>
